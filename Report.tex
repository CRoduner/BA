...
\section{Introduction}
	...
	\todo{Einheitliche Schreibweise der NSG (mit/ohne Bindestrich, usw).}
...
\section{Physical background}
	...
	\subsection{Navier-Stokes and Continuity Equation}
		...
		In component form the Navier-Stokes equation in cylindrical coordinates is:
		\todoWriteMore{write down equations in cylindrical coordinates in component form and some text with it (more explanation why it was time consuming)}
...
\section{Numerical Solution
	...
	\subsection{Discretization} % Mehr einleitender Text vor Unterkapitel
		...
		\subsubsection{Navier-Stokes equation}
			...
			Applying the forward finite difference on the left-hand side of equation \todoRef{Ref auf Komponentenform der NSG} and solving for $u'(r,\phi,z) = u(\vec{r}, t+\delta t)$, $v'(r,\phi,z) = v(\vec{r}, t+\delta t)$ and $w'(r,\phi,z) = w(\vec{r}, t+\delta t)$, hands us the following discrete equations:
			\todoFormat{u\_p durch u ersetzen?}
			...
			\begin{eqnarray}
				% v_p[i,j,k] = \frac{1}{2}\left(v[i+1,j,k]+v[i,j,k]\right) %-> kann man verwenden, macht aber nicht so sinn
	...
	\subsection{Pressure readjustment} % Evtl. mehr Erklärung, wieso diese Methode gewählt wurde
		...
		A Poisson equation can be solved with a modified Richardson iteration. \todoRef{Fluiddynamik und Wiki}%\cite{WikiRichardson} % hier Referenz auf Wikipedia (en.wikipedia.org/wiki/Modified_Richardson_iteration)
	...
	\subsection{CFL-Criterion} % Referenz auf Buch und wenn möglich, etwas mehr Erklärung
		...
		which must be satisfied, in order to obtain stable numerical solutions using central differences. It links the velocity of the fluid to the resolution of the space-time grid required to resolve the flow. \todoRef{Stocker} %\cite{Stocker2009}
