...
pnew = np.zeroes((n_r,n_phi,n_z))

grad_u = np.zeroes((n_r,n_phi,n_z))
...
for i in range(1,n_r-2):
    ...
    for j in range(n_phi):
        ...
        for k in range(1,n_z-1):
            ...
            grad_u[i,j,k
            pnew[i,j,k] = p[i,j,k] + lamb*(rpi*0.5*(u[i-1,j,k]+u[i,j,k]) + dri*(u[i,j,k]-u[i-1,j,k]) + rpi*dphii*(v[i,j,k]-v[i,j-1,k]) +
